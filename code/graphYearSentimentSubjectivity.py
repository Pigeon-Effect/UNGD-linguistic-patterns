...
def calculate_sentiment_subjectivity(text):
    """Calculate the sentiment subjectivity for the given text using TextBlob."""
    blob = TextBlob(text)
    return blob.sentiment.subjectivity


def process_folder_by_session(folder_path):
    """Process each session folder, calculate average sentiment subjectivity per year."""
        ...
        if match:
            ...
            for txt_file in txt_files:
                with open(txt_file, 'r', encoding='utf-8') as file:
                    ...
                    # Calculate the sentiment subjectivity for the text
                    sentiment_subjectivity = calculate_sentiment_subjectivity(preprocessed_content)
                    year_sentiment_subjectivities.append(sentiment_subjectivity)

            # Calculate the average sentiment subjectivity for the year
            if year_sentiment_subjectivities:
                avg_sentiment_subjectivity = sum(year_sentiment_subjectivities) / len(year_sentiment_subjectivities)
                sentiment_subjectivities.append(avg_sentiment_subjectivity)
                years.append(year)
            else:
    ...
    return years, sentiment_subjectivities


def plot_sentiment_subjectivity_timeline(years, sentiment_subjectivities, output_path):
    if not years or not sentiment_subjectivities:
        print("No data to plot.")
        return

    fig, ax = plt.subplots(figsize=(10, 3))
    ax.plot(years, sentiment_subjectivities, marker='o', label='Sentiment Subjectivity')

    # Polynomial regression (e.g., a degree 5 polynomial)
    polynomial_degree = 5
    coefficients = np.polyfit(years, sentiment_subjectivities, polynomial_degree)
    ...
    ax.set_title('S5 - Sentiment Subjectivity from 1946 to 2022', pad=20)
...
output_path = r'C:\Users\juliu\Desktop\DesinformationTermPaper\static\SVGs\s5_sentiment_subjectivity_1946_2022.svg'  # Path to save the SVG file
years, sentiment_subjectivities = process_folder_by_session(folder_path)
plot_sentiment_subjectivity_timeline(years, sentiment_subjectivities, output_path)
